...
const MovementForm: React.FC<Props> = ({ items, onAddMovement }) => {
  ...
  const inputWrapper = "group flex items-center w-full rounded-xl bg-slate-50 border border-slate-200 focus-within:bg-white focus-within:border-indigo-500 focus-within:ring-4 focus-within:ring-indigo-500/10 transition-all h-11"
  const inputField = "w-full h-full bg-transparent border-none text-sm text-slate-700 placeholder:text-slate-400 focus:ring-0 px-3 outline-none disabled:opacity-50"
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 h-full flex flex-col">
      <div className="mb-6">
        ...
        <p className="text-xs text-slate-400">Nova movimentação de estoque.</p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-5 flex-1 flex flex-col">
        {/* Toggle Entrada/Saída */}
        <div className="bg-slate-50 p-1 rounded-xl flex border border-slate-100 relative">
          <button 
            type="button" 
            onClick={() => setType('entrada')} 
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-xs font-bold rounded-lg transition-all duration-200 ${type === 'entrada' ? 'bg-white text-emerald-700 shadow-sm ring-1 ring-black/5' : 'text-slate-400 hover:text-slate-600'}`}
          >
          ...
          <button 
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-xs font-bold rounded-lg transition-all duration-200 ${type === 'saida' ? 'bg-white text-rose-600 shadow-sm ring-1 ring-black/5' : 'text-slate-400 hover:text-slate-600'}`}
          >
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <div className={inputWrapper}>
               ...
               <input type="date" value={date} onChange={e => setDate(e.target.value)} className={inputField} disabled={disabled} />
            </div>
          </div>
          <div>
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          {showDropdown && !disabled && (
            <div className="absolute z-30 mt-1 w-full bg-white rounded-xl shadow-xl border border-slate-100 max-h-56 overflow-y-auto custom-scrollbar animate-in fade-in zoom-in-95 duration-100">
              {filteredItems.length > 0 ? (
                filteredItems.map((item) => (
                  <button
                    key={item.id} type="button" onClick={() => handleSelectItem(item)}
                    className="w-full text-left px-4 py-3 text-xs font-medium text-slate-600 hover:bg-indigo-50 hover:text-indigo-700 border-b border-slate-50 last:border-0 transition-colors flex items-center justify-between"
                  >
                    <span className="truncate">{item.description}</span>
                    {item.id === selectedItemId && <Check size={14} className="text-indigo-600" />}
                  </button>
                ))
              ) : (
        ...
        <div className="mt-auto pt-2">
            <button 
                type="submit" 
                disabled={disabled} 
                className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-3.5 rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-slate-200 active:scale-[0.98] flex items-center justify-center gap-2"
            >
